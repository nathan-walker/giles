	...
	checkRobots(protocol, host, path) {
		...
		return new Promise((resolve, reject) => {
			...
			(err, robotstxt) => {
				...
				if (robotstxt) {
					resolve(robots.checkPathFromSerialized(JSON.parse(robotstxt), path));
					return;
				}
				
				// If not available, call fetchRobots()
				return this.fetchRobots(protocol, host, path);
	...
	fetchRobots(protocol, host, path) {
		...
		return new Promise((resolve) => {
			...
			robotsRequest.get(protocol + "//" + host + "/robots.txt", (err, res, body) => {
				...
				if (err || res.statusCode !== 200 || !body) serialized = ['1^\/'];
				else {
					var rules = robots.parse(body);
					if (!rules) serialized = ['1/^\//'];
					else {
						serialized = robots.serializeRules(rules);
				...
				this.redis.setex(this.redisKey + "robots:" + protocol + host, 60 * 60 * 24, JSON.stringify(serialized));
